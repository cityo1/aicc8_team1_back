...
export const checkDeficiency = async (req, res) => {
    try {
        ...
        if (!userId || !date) {
            return res.status(400).json({ success: false, message: "userId와 date가 필요합니다." });
        }

        // 1. 해당 날짜의 섭취 영양소 합계 계산
        const query = `
            SELECT 
                SUM(f.calories * de.amount) as total_calories,
                SUM(f.carbohydrate * de.amount) as total_carbohydrate,
                SUM(f.protein * de.amount) as total_protein,
                SUM(f.fat * de.amount) as total_fat
        ...
        if (nutrition.total_calories < thresholds.calories) {
            alerts.push({ type: "CALORIES", current: nutrition.total_calories, target: thresholds.calories });
        }
        if (nutrition.total_protein < thresholds.protein) {
            alerts.push({ type: "PROTEIN", current: nutrition.total_protein, target: thresholds.protein });
        }
